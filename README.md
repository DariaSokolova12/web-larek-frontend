...
Данные:
- `basketItems: IProductItem[]` - список товаров в корзине.
...
Данные:
- `productList: IProductItem[]` - список товаров.
- `previewProduct: IProductItem | null` - текущий выбранный товар.

Метод:
- `setPreview(product: IProductItem)` - получает данные карточки которую открыл пользователь.

### Класс `FormModel` хранит данные полученные от пользователя.

Данные:
- `orderData: IOrderForm` - данные формы заказа.

Методы:
- `setOrderAddress(address: string)` - принимаем/сохраняет адрес пользователя.
- `validateOrder` - проверяет адрес пользователя / и способ оплаты.
- `setOrderData(phone: string, email: string)` - принимаем/сохраняет номер телефона/почту пользователя.
...
Методы:
- `renderHeaderBasketCounter(count: number)` - сохраняет и устанавливает какое количество товаров находится в корзине.
- `renderSumAllProducts(sum: number)` - сохраняет и устанавливает сумму синапсов всех товаров в корзине.

### Класс `BasketItem` управляет отображением элементов(продуктов) в корзине.

метод:
- `setPrice(price: number): string` - принимает цену продукта в числовом значении и возвращает в строчном.

### Класс `Card` управляет отображением карточки товара на веб странице.

Методы:
- `setText(element: HTMLElement, text: string)` - принимает два значения, первое HTMLElement, второе значение задаёт текстовое содержимое HTMLElement.
...
Метод:
- `notSale(product: IProductItem)` - принимает данные о продукте, проверяет наличие цены продукта, при отсутствии цены ограничивает покупку.

### Класс `Order` управляет отображением содержимого модального окна и позволяет принять от пользователя метод оплаты и адрес.

Метод:
- `paymentSelection(method: string)` - устанавливаем обводку вокруг выбранного метода оплаты.
...
- `contentElement: HTMLElement` - элемент для рендеринга содержимого (корзина, форма заказа и т. д.).
