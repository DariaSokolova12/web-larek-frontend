...
export class AppState extends Model<IAppData> {
	...
	getTotal() {
	   return this.basketItem.reduce((a, c) => a + (c.price || 0), 0);
	}
	...
	// Новый метод для формирования payload перед отправкой
	getOrderPayload(): { items: ICardProduct[], total: number, address: string, payment: string, email: string, phone: string } {
	   return {
		  items: this.basketItem,
		  total: this.getTotal(),
